...
class OrderController extends Controller
{
    ...
    public function newOrder(int $id = 0)
    {
        ...
        $idRate = $response->collect()->firstWhere('service', '=', $id);
        $idRate = $idRate['rate'] * 3.5;

        return view('orders.new-order', ['id' => $id, 'idRate' => $idRate, 'data' => $response->json(), 'balance' => $this->paymentRepository->getPaymentBalanceByUserId(Auth::user()->id)]);
    }
